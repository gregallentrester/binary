...
import net.greg.examples.mockito.MyList;
...
public class MockitoMockUnitTest {
  ...
  public void whenUsingMockWithName_thenCorrect() {
    ...
    MyList listMock =
      mock(MyList.class, "myMock");

    when(listMock.add(anyString())).thenReturn(false);

    listMock.add(randomAlphabetic(6));

    expectedException.expect(TooLittleActualInvocations.class);
    expectedException.expectMessage(containsString("myMock.add"));
  ...
  public void whenUsingMockWithAnswer_thenCorrect() {
    ...
    MyList listMock =
      mock(MyList.class, new CustomAnswer());
  ...
  public void whenUsingMockWithSettings_thenCorrect() {
    ...
    MockSettings customSettings =
      withSettings().defaultAnswer(new CustomAnswer());

    MyList listMock =
      mock(MyList.class, customSettings);
